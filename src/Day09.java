...
public class Day09 {
    ...
    private final List<Pair> rules = new ArrayList<>();
...
//        String input = getInput();
        ...
        System.out.println("Part 1: " + totalValid);
        System.out.println("Part 2: " + totalInvalid);
    }

    private void parseUpdate(String line) {
        String[] tokenSplit = line.trim().split(",");
        List<Long> tokens = Arrays.stream(tokenSplit).map(m -> Long.parseLong(m)).toList();

        boolean isNotValid = true;
        for (int i = 0; i < tokens.size(); i++) {
            if (!isValidToken(i, tokens)) {
                isNotValid = false;
                break;
            }
        }
        ...
        if (isNotValid) {
            totalValid += tokens.get(tokens.size() / 2);
        } else {
            List<Long> reorderedTokens = reorderTokens(new ArrayList<>(tokens));
            totalInvalid += reorderedTokens.get(reorderedTokens.size() / 2);
        }
    }

    private void parseRule(String line) {
        String[] tokens = line.trim().split("\\|");
        long num1 = Integer.parseInt(tokens[0]);
        long num2 = Integer.parseInt(tokens[1]);
        rules.add(new Pair(num1, num2));
    }

    private List<Long> reorderTokens(List<Long> tokens) {
        index:
        for (int i = 0; i < tokens.size(); i++) {
            long t = tokens.get(i);
            rules:
            for (Pair e : rules) {
                if (e.after == t) {
                    for (int j = 0; j < tokens.size(); j++) {
                        long t2 = tokens.get(j);
                        if (e.before == t2 && j < i) {
                            continue rules; // found key before our value, all good, let's move onto next rule!
                        } else if (e.before == t2) {
                            // Found key after value, swap the items and reset i.
                            tokens.set(i, t2);
                            tokens.set(j, t2);
                            i = -1;
                            continue index;
    ...
    private boolean isValidToken(int i, List<Long> tokens) {
        long t = tokens.get(i);
        rules:
        for (Pair e : rules) {
            if (e.after == t) {
                for (int j = 0; j < tokens.size(); j++) {
                    ...
                    if (e.before == t2 && j < i) {
                        ...
                        // found key after our value, not good
                        return false;
                    }
                }
            }
